...
// Copyright 2014 Kevin Massaroni
// Author: Kevin Massaroni
// Adapted from the original work by David Helder.
...
service :
  ...
  } extends? '{' function* '}' typeAnnotations?
  {this.serviceName = null;} -> ^(SERVICE IDENTIFIER extends? function* typeAnnotations?);

extends:
  'extends' IDENTIFIER -> ^(EXTENDS IDENTIFIER);

function:
  oneway? functionType name=IDENTIFIER {
  	this.functionName = name.getText();
  	this.services[this.serviceName][this.functionName] = [];
  } '(' field* ')' throwz? typeAnnotations? commaOrSemicolon? {this.functionName = null;}
